...
Window_StatCompare.prototype.refresh = function() {
    this.contents.clear();
    if (!this._actor) return;
    for (var i = 0; i < 7; ++i) { // increased from 8 to 12
        this.drawItem(0, this.lineHeight() * i, i);
    }
};

Window_StatCompare.prototype.paramArray = function(actor, paramId) {
    switch (paramId){
        case 0: return ["\\I[1]MaxHP",actor.mhp]; break;
        //case 1: return ["\\I[55]Max MP",actor.mmp]; break;
        case 1: return ["\\I[2]MaxAP",actor.agi]; break;
        case 2: return ["\\I[12]HP+",(actor.hrg<0 ? "":"+")+(Math.round(actor.hrg*1e3)/1e1)]; break;
        //case 4: return ["\\I[1076]MP Regen",(actor.mrg<0 ? "":"+")+(Math.round(actor.mrg*1e3)/1e1)]; break;
        case 3: return ["\\I[13]TP+",(actor.trg<0 ? "":"+")+(Math.round(actor.trg*1e3)/1e1)]; break;
        //case 6: return ["\\I[57]Attack",actor.atk]; break;
        //case 7: return ["\\I[58]Technical",actor.def]; break;
        case 4: return ["\\I[9]Cost",actor.luk]; break;
        case 5: return ["\\I[10]Hit",(Math.round(actor.hit*1e3)/1e1)]; break;
        case 6: return ["\\I[11]Evade",(Math.round(actor.eva*1e3)/1e1)]; break;
        //case 11: return ["\\I[1060]Resist",(Math.round(actor.mat*1e1)/1e1)]; break;
        default: return [];
    }
}
...
Window_StatCompare.prototype.drawParamName = function(y, paramId) {
    var x = this.textPadding();
    this.changeTextColor(this.systemColor());
    this.drawTextEx(this.paramArray(this._actor,paramId)[0], x, y, this._paramNameWidth);
};
...
Window_StatCompare.prototype.drawCurrentParam = function(y, paramId) {
    ...
    x -= this._paramValueWidth * 2 + this._arrowWidth + this._bonusValueWidth;
    this.resetTextColor();
    var actorparam = Yanfly.Util.toGroup(this.paramArray(this._actor,paramId)[1]);
    //if(!actorparam) return;
    if (paramId===5||paramId===6)actorparam+="%";
    this.drawText(actorparam, x, y, this._paramValueWidth, 'right');
};
...
Window_StatCompare.prototype.drawNewParam = function(y, paramId) {
    var x = this.contents.width - this.textPadding();
    x -= this._paramValueWidth + this._bonusValueWidth;
    var newValue = this.paramArray(this._tempActor,paramId)[1];
    var diffvalue = newValue - this.paramArray(this._actor,paramId)[1];
    var actorparam = Yanfly.Util.toGroup(newValue);
    if (paramId===5||paramId===6)actorparam+="%";
    if(paramId===4) diffvalue=-diffvalue;
    this.changeTextColor(this.paramchangeTextColor(diffvalue));
    if(paramId===4) diffvalue=-diffvalue;
    this.drawText(actorparam, x, y, this._paramValueWidth, 'right');
};
...
/* Lower the width of the equip menu
--------------------------------------------------------------------*/
Window_EquipCommand.prototype.windowWidth = function() {
    return 140;
};