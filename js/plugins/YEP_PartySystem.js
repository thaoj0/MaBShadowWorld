...
Window_PartyMenuCommand.prototype.windowWidth = function() {
    ...
    // Edited above to lower party menu's menu width MAB
    return (Graphics.boxWidth - ww).clamp(Graphics.boxWidth / 4, 140);
};
...
Window_PartyMenuCommand.prototype.addChangeCommand = function() { // <AB rebuild party menu
    if (Yanfly.Param.PartyCommand1 === '') return;
    this.addCommand("Equipment", 'equip');
    this.addCommand("Active Skills", 'skill');
...
Window_PartyList.prototype.initialize = function(partyWindow) {
    ...
    var ww = this.windowWidth(); // MAB controls the Order width
...
Window_PartyList.prototype.activate = function() {
    ...
    this.reselect();
    this.show(); // MAB added to show box when in use
};

Window_PartyList.prototype.deactivate = function() {
    Window_Base.prototype.deactivate.call(this);
    this.reselect();
    this.hide(); // MAB added to hide box when not in use
};
...
Window_PartyList.prototype.setDetailWindowItem = function(actor) {
    if (this._detailWindow) this._detailWindow.setActor(actor); 
    if (this._detailWindow) this._detailWindow._createsprite.visible = false;
};
...
Window_PartyDetail.prototype.initialize = function(x, y, width, height) {
    ...
    this._createsprite = new Sprite('');
    this.addChild(this._createsprite);
    this._actor = null;
};

Window_PartyDetail.prototype.setActor = function(actor) {
    this._createsprite.visible = true;
...
Window_PartyDetail.prototype.clear = function() {
    ...
    this.removeChild(this._createsprite);
};
...
Window_PartyDetail.prototype.drawActorBasicInfo = function() {
    ...
    this.removeChild(this._createsprite);
    this._createsprite = new Sprite(ImageManager.loadPicture(this._actor.name() + '_G')); 
    this._createsprite.x -= 300;
    //this._createsprite.x -= (this.width)/2 - this.padding * 2; // MAB moved dynamicly
    this.addChild(this._createsprite);
};
...
Window_PartyDetail.prototype.drawActorParams = function() {
    ...
    var arrayname = ["\\I[54]","\\I[55]","\\I[56]","\\I[57]","\\I[58]","\\I[62]"];
    var paramIds = [0,1,6,2,3,7];
    for (var i = 0; i < 6; ++i) {
      var rect = this.paramRect(i);
      ...
      //var paramId = i + 2;
      var text = TextManager.param(paramIds[i]);
      this.changeTextColor(this.systemColor());
      //this.drawText(text, rect.x, rect.y, rect.width);
      this.drawTextEx(arrayname[i] + text, rect.x, rect.y, rect.width);
      var paramValue = Yanfly.Util.toGroup(this._actor.param(paramIds[i]));
...
Window_PartyDetail.prototype.drawActorEquips = function() {
    ...
    arrayname = ["\\I[1075]HP Regen","\\I[1076]MP Regen","\\I[1077]TP Regen","\\I[1056]Hit","\\I[1058]Evade","\\I[1060]Resist"];
    arrayvalue = [this._actor.hrg,this._actor.mrg,this._actor.trg,this._actor.hit,this._actor.eva,this._actor.mat*0.01];
    for (var i = 0; i < 6; ++i) {
      var rect = this.paramRect(i);
      if (this._linesAvailable > 4) {
      ...
      this.changeTextColor(this.systemColor());
      //this.drawText(text, rect.x, rect.y, rect.width);
      this.drawTextEx(arrayname[i], rect.x + rect.width/2, rect.y, rect.width);
      var paramValue = Yanfly.Util.toGroup(this._actor.param(paramId));
      this.changeTextColor(this.normalColor());
      var sign = "";
      if(i<3)sign = arrayvalue[i]<0 ? "":"+";
      this.drawText(sign+(Math.round(arrayvalue[i]*1e3)/1e1)+(i>=3 ? "%":""), rect.x + rect.width/2, rect.y, rect.width, 'right');
      //this.drawTextEx(arrayvalue[i].toString(), rect.x + rect.width - 24, rect.y, rect.width);
      //this.drawGaugeBars(rect.x + rect.width*1.5 - 24, rect.y, 6, arrayvalue[i]*10, this.textColor(29), this.textColor(15));
    }
...
Window_PartyDetail.prototype.drawActorParams = function() {
    this.drawActorParamsTitle();
    var arrayname = ["\\I[1]","\\I[2]","\\I[39]","\\I[9]","",""];
    var paramIds = [0,6,1,7,2,3];
    for (var i = 0; i < 4; ++i) {
      var rect = this.paramRect(i);
      if (this._linesAvailable > 4) {
        rect.x += 8;
...
Window_PartyDetail.prototype.drawActorEquips = function() {
    this.drawActorEquipsTitle();
    arrayname = ["\\I[12]HP Regen","\\I[13]AP Regen","\\I[10]Hit","\\I[11]Evade","",""];
    arrayvalue = [this._actor.hrg,this._actor.trg,this._actor.mrg,this._actor.hit,this._actor.eva,this._actor.mat*0.01];
    for (var i = 0; i < 4; ++i) {
      var rect = this.paramRect(i);
      if (this._linesAvailable > 4) {
        rect.x += this.lineHeight() * 7;
      ...
      this.changeTextColor(this.normalColor());
      var sign = "";
      if(i<2)sign = arrayvalue[i]<0 ? "":"+";
      this.drawText(sign+(Math.round(arrayvalue[i]*1e3)/1e1)+(i>=2 ? "%":""), rect.x + rect.width/2, rect.y, rect.width, 'right');
      //this.drawTextEx(arrayvalue[i].toString(), rect.x + rect.width - 24, rect.y, rect.width);
      //this.drawGaugeBars(rect.x + rect.width*1.5 - 24, rect.y, 6, arrayvalue[i]*10, this.textColor(29), this.textColor(15));
    }
