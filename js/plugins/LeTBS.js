...
InputHandlerTBS.update = function () {
    ...
    if (Input.isTriggered("pageup"))
        this.processCallback("PageUp");
    if (Input.isTriggered("pagedown"))
        this.processCallback("PageDown");
...
BattleManagerTBS.setStartBattleSwitch = function (value) {
    $gameParty.allMembers().forEach(function(actor){
        actor._hp = actor.mhp;
        actor._mp = actor.mmp;
    });
    this.startBattleSwitch = value;
};

BattleManagerTBS.getStartBattleSwitch = function () {
    if(this.startBattleSwitch==undefined) return false;
    return this.startBattleSwitch;
};
...
BattleManagerTBS.determineTurnOrder = function () {
    if (Lecode.S_TBS.turnOrderFairDistribution || $gameTroop.turnCount() === 0)
...
BattleManagerTBS.getScopeBoundaries = function (cells) {
    ...
    if (cellsByX.length === 0) return {} // This allows for scopes to not exist
...
BattleManagerTBS.getEntityByEnemyIdAndIndex = function (enemyId, index) {
    return this.getEntitiesByEnemyId(enemyId)[index];
};
...
BattleManagerTBS.evaluateSequenceConditions = function (seqMng, str) {
    ...
    var isTarget = function (entityTargets) {
    ...
    var hasState = function(entity, stateId) {
        var entities = seqMng.readTargets(entity);
        return entities[0].battler().isStateAffected(stateId);
    }
...
TBSAiManager.prototype.processBehaviors = function () {
    if (!this._battler.canMove() || this._battler.tp <= 0) {
        return;
    }
...
TBSAiManager.prototype.func_CanUseSupport = function () {
    //if (this._entity._aiSupportCount !== 0) return false; // double cooldown mab
    if (this.getUsableSkills(this._battler).length === 0) {
        this._failureCode = "no_usable_skill";
        return false;
    }
...
TBSEntity_Sprite.prototype.updateBitmap = function () {
    ...
    if(this.bitmap==undefined)console.log("this.bitmap is undefined: "+this);
...
Game_Interpreter.prototype.readEntityForLeTBS = function (data) {
    ...
    else if (data.match(/^member_actor\((.+)\)$/i)) {
        return BattleManagerTBS.getEntityByActorMember(Number(RegExp.$1));
    }
    else if (data.match(/^actor\((.+)\)$/i)) {
        return BattleManagerTBS.getEntityByActorId(Number(RegExp.$1));
    }
    else if (data.match(/^enemy_index\((.+)\,(.+)\)$/i)) {
        return BattleManagerTBS.getEntityByEnemyIdAndIndex(Number(RegExp.$1),Number(RegExp.$2));
    }
...
DataManager.processLeTBSTagsForBattlers = function () {
    ...
    for (var i = 0; i < groups.length; i++) {
        ...
        for (var j = 1; j < group.length; j++) {
            ...
            obj.TagsLetbs.hideHudWindow = false;
            ...
            for (var k = 0; k < notedata.length; k++) {
                ...
                if (letbs) {
                    ...
                    else if (line.match(/hide_hud_window/i))
                        obj.TagsLetbs.hideHudWindow = true;
...
// This is very, very tricky. I recommending testing everything.
//-------------------------------------------------------------------------
TBSEntity.prototype.onCellCovered = function () {
    var stop = BattleManagerTBS.executeEventsByStep(this);
    // When returning true, the movement is stoped
    this._movingNextCell = null;
    if (this.getMovePoints() === 0) return true;
    if (this._moveReducePoints) {
        //this._movePoints--;
        var moveCost = this.battler().luk;
        var a = this.battler();
        var tpPop = true;
        
        if (this.battler().isStateAffected(61) && Math.random() < 0.30) { // Ninja moveCost ignore
            moveCost = 0;
            //tpPop = false;
            this.addTextIconPopup("0",0,4);
        }
        if (this.battler().isStateAffected(2)) { // Burn Debuff 
            a.gainHp(-2);
            //tpPop = false;
            this.addTextIconPopup("-"+2,0,3);
        }
        if (this.battler().isStateAffected(50)) { // Dancer State
            var get = a.getStateCounter(50)||0; 
            a.setStateCounter(50, get + moveCost);
            a.addParam(2,moveCost);
            a.addParam(3,moveCost);
        }
        if (this.battler().isStateAffected(103)) { // Thermal Aura
            var get = a.getStateCounter(103)||0; 
            a.setStateCounter(103, get + 1);
            a.addParam(2,1);
            a.addParam(3,1);
        }
        if ( this.battler().isStateAffected(34) ){ // This generates rush
            var get = a.getStateCounter(34)||0; 
            a.setStateCounter(34, get + 1);
        };
        
        if( this.battler().isStateAffected(19) ){ // Quicken Buff
            a.setMobileEvasion(1); 
        };
        if( this.battler().isStateAffected(76) ){ // Cloth gives evasion per move
            a.setMobileEvasion(2); 
        };
        if( this.battler().isStateAffected(77) ){ // Medium gives evasion per move
            a.setMobileEvasion(1); 
        };
        if ( this.battler().isStateAffected(48) ){ // Renji gets evasion per move regardless
            a.setMobileEvasion(1);
        };
        
        if(moveCost != 0) this.battler().gainTp(-moveCost);
        if(tpPop) this.addTextIconPopup("-"+moveCost,0,4);
        if (Math.floor(this.battler().tp/moveCost) === 0) return true;
        if (this.getMovePoints() === 0) return true;
    }
    return stop;
};
...
TBSEntity.prototype.getMovePoints = function () {
    ...
    if (this.playableByAI()){ // An AI will always save 3 TP
        calTP -= 3;
    }
    return Math.max(0,Math.floor(calTP));
    //return this._movePoints.clamp(0, this._movePoints);
};
// move is always active so you can use it to scan
TBSEntity.prototype.canMoveCommand = function () { 
    if (!this.playableByAI()) return true;
    if (this.getMovePoints() === 0) return false;
    return true;
};

// Can move through allies but not neutrals
TBSEntity.prototype.isPassable = function () {
    if (this._isNeutral) return false; // Noone can pass neutral not even neutral
    if (this.battler().hasLeTBSTag("notPassable")) return false;
    if (BattleManagerTBS.activeEntity().battler().isStateAffected(61)) return true; // If has state Ninja
    if (BattleManagerTBS.activeEntity().battler().isStateAffected(111)) return true; // If has state Reaper
    return (this._battler.isDead() && this.isPassableOnDeath()) || 
        this.battler().hasLeTBSTag("passable") || 
        LeUtilities.isAlly(this.battler(), BattleManagerTBS.activeEntity().battler());
};
//-------------------------------------------------------------------------
...
TBSSequenceManager.prototype.update = function () {
    ...
    this._sequence.table.push(null); // This keeps the seq running even when empty
    var command = this._sequence.table.shift();
    if (command){
        this.runCommand(command);
    } else {
...
BattleManagerTBS.onTurnOrderEnd = function () {
    $gameTroop.increaseTurn();
    this.executeEventsWhen("turn_order_end");
    BattleManagerTBS.determineTurnOrder(); // Added to create a dynamic turn order because of tp=agi
    
    // field turn order ticks at the end of every order turn
    for(var i = 0; i < BattleManagerTBS._marksManager._marks.length; i++){
        var fieldMark = BattleManagerTBS._marksManager._marks[i];
        if(fieldMark._id.slice(-5) === "field"){
            this.updateMarksDuration(fieldMark._user, "start");
            break;
        }
    }
};
...
- Player always has first turn advantage
- First turn is always fair
- turn order is determined by amount of tp left
******************************************************************************/
BattleManagerTBS.determineTurnOrderFair = function () {
    var array = [];
    var actors = [];
...
BattleManagerTBS.determineTurnOrderSimple = function () {
    ...
    array = array.sort(function (a, b) {
        return b._battler.tp - a._battler.tp; // Turn order is determined by tp instead of agi
    });
...
Game_BattlerBase.prototype.skillTpCost = function(skill) { // TP is also affected by mcr
    return Math.floor(skill.tpCost * this.mcr);
    //return skill.tpCost;
};
...
// Notes: 
// - position switching skills such as leaps and swap's attackedFrom() are set from original pos
//-------------------------------------------------------------------------

Game_Action.prototype.itemCnt = function(target) {
    if (target.canMove()) { // && target.attackedFrom()!="back") { // no counter from back
        return target.cnt;
    } else {
        return 0;
    }
...
/** This creates a new stat called temp stat
-------------------------------------------------------------------------*/
Game_BattlerBase.prototype.mapTStat = function(stat){ // This maps stat to array
    switch(stat){
        case 'maxhp': return this._tempMaxhp;
        case 'maxmp': return this._tempMaxmp;
        case 'atk': return this._tempAtk;
        case 'def': return this._tempDef;
        case 'mat': return this._tempMat;
        case 'agi': return this._tempAgi;
        case 'luk': return this._tempLuk;
        case 'hit': return this._tempHit;
        case 'eva': return this._tempEva;
        case 'cev': return this._tempCev;
        case 'cnt': return this._tempCnt;
        case 'hpreg': return this._tempReghp;
        case 'mpreg': return this._tempRegmp;
        case 'tpreg': return this._tempRegtp;
        default: console.log(stat + ": mapTStat() no match");
    }
    return [];
}
Game_BattlerBase.prototype.setTStat = function(stat, array){ // this saves array to original
    switch(stat){
        case 'maxhp': this._tempMaxhp = array; break;
        case 'maxmp': this._tempMaxmp = array; break;
        case 'atk': this._tempAtk = array; break;
        case 'def': this._tempDef = array; break;
        case 'mat': this._tempMat = array; break;
        case 'agi': this._tempAgi = array; break;
        case 'luk': this._tempLuk = array; break;
        case 'hit': this._tempHit = array; break;
        case 'eva': this._tempEva = array; break;
        case 'cev': this._tempCev = array; break;
        case 'cnt': this._tempCnt = array; break;
        case 'hpreg': this._tempReghp = array; break;
        case 'mpreg': this._tempRegmp = array; break;
        case 'tpreg': this._tempRegtp = array; break;
        default: console.log("setTStat() no match");
    }
    this.refresh();
}
Game_BattlerBase.prototype.setupTempStat = function(){ // this creates the base stat
    this._tempMaxhp = [];
    this._tempMaxmp = [];
    this._tempAtk = [];
    this._tempDef = [];
...
// Get temp value total
Game_BattlerBase.prototype.getTempStat = function(stat) {
    var total = 0;
    var array = this.mapTStat(stat);
    ...
    //array.forEach(function(e){total+=e[1]});
    return total;
};
// This is an array similar to ruby's array
Game_BattlerBase.prototype.setTempStat = function(stat, value) {
    var key = "";
    var array = this.mapTStat(stat);
    
    if(Array.isArray(value)){ // if array push value
        key = value[0];
        value = value[1];
    }
    if (array.filter(function(e){return e[0]===key;}).length > 0){
        array = array.map(function(e){return (e[0]===key) ? [e[0],value] : e;});
    }else{
        array.push([key,value]);
    };
    this.setTStat(stat,array);
};

/** This set up weapon temp damage a rebuilt of tempStat 
------------------------------------------------------------------------------------------*/
Game_BattlerBase.prototype.setTempDamage = function(stat, value) {
    // curse makes your weapon break -1 faster
    if(this.isStateAffected(7) && value < 0) value-=1;
    if(this.isStateAffected(93) && value < 0) value-=1;
    
    var e = this.mapTStat(stat);
    var get = 0;
    ...
    //console.log(get);
    this.setTempStat(stat,["TempDamage",get + value]);
};
Game_BattlerBase.prototype.getTempDamage = function(stat) {
    var e = this.mapTStat(stat);
    var get = 0;
    for(var i=0;i<e.length;i++){
        if(e[i][0]==="TempDamage"){
...
Game_BattlerBase.prototype.clearTempDamage = function(stat) {
    this.setTempStat(stat,["TempDamage",0]);
};

/** This set up mobile evasion system 
- each move wil increases your evasion rate
- removed at turn begin
------------------------------------------------------------------------------------------*/
Game_BattlerBase.prototype.setMobileEvasion = function(value) {
    this.addState(72);
    var get = this.getStateCounter(72)||0; 
    this.setStateCounter(72, Math.max(get + value,0));
};
Game_BattlerBase.prototype.getMobileEvasion = function() {
    if (!this.isStateAffected(72)) return 0;
    return this.getStateCounter(72)||0;
};
Game_BattlerBase.prototype.clearMobileEvasion = function() {
    if (this.isStateAffected(72)) {
        this.setStateCounter(72, 0);
        this.refresh();
    }
};

/** This set up active hit system 
- each action will decrease your hit rate
- removed at turn end

--- Currently Not Used ---
------------------------------------------------------------------------------------------*/
Game_BattlerBase.prototype.setActiveHit = function(value) {
    this.addState(71);
    var get = this.getStateCounter(71)||0; 
    this.setStateCounter(71, Math.min(get + value,0));
};
Game_BattlerBase.prototype.getActiveHit = function() {
    if (!this.isStateAffected(71)) return 0;
    return this.getStateCounter(71)||0;
};
Game_BattlerBase.prototype.clearActiveHit = function() {
    if (this.isStateAffected(71)) {
        this.setStateCounter(71, 0);
        this.refresh();
    }
};

/** This adds damage skill resistance to the game 
-When poison double resist lost
------------------------------------------------------------------------------------------*/
/*
Game_Action.prototype.applyDamageSkillCost = function(value, target) {
    var a = this.subject();
    var b = target;
    var cost = (a.skillMpCost(this.item())||0) + (a.skillTpCost(this.item())||0);
    if(b.isStateAffected(4)) cost*=2; // When poison double resist lost
    var e = b.mapTStat('mat');
    var get = 0;
    for(var i=0;i<e.length;i++){
        if(e[i][0]==="applyDamageSkillCost"){
            get = e[i][1];
            break;
        }
    }
    //console.log( (value + Math.max(cost - b.mat,0)) + "=" + value + "+" + cost + "-" +b.mat);
    value += Math.max(cost - b.mat,0);
    value = Math.max(0, value);
    b.setTempStat('mat',["applyDamageSkillCost",get - cost]);
    return value;
};*/

/** This creates a new method to capture Total HP/SP/MP before paying cost
These methods are purely used for "current hp/sp/mp" skills that also cost hp/sp/mp
------------------------------------------------------------------------------------------*/
Game_BattlerBase.prototype.prePayHp = function() {
    return this._prePayHp;
};
Game_BattlerBase.prototype.prePayMp = function() {
    return this._prePayMp;
};
Game_BattlerBase.prototype.prePayTp = function() {
    return this._prePayTp;
};
Game_BattlerBase.prototype.setPrePayStats = function() {
    this._prePayHp = this.hp;
    this._prePayMp = this.mp;
    this._prePayTp = this.tp;
};
var PrePay_Game_BattlerBase_prototype_paySkillCost = Game_BattlerBase.prototype.paySkillCost;
Game_BattlerBase.prototype.paySkillCost = function(skill) {
    this.setPrePayStats();
    PrePay_Game_BattlerBase_prototype_paySkillCost.call(this, skill);
};

/** Dodge rate - is based on user.hp - target.eva
Attacks ignored eva if from behind
Attacks ignored eva target is Immoblized
- Actor has a double chance to evade (Purely for "balance")
------------------------------------------------------------------------------------------*/
Game_Action.prototype.itemHit = function(target) {
    if (!this.isPhysical()) return this.item().successRate * 0.01;
    var hit = this.subject().hit;
    var eva = (target.attackedFrom()=="back" || target.isStateAffected(8)) ? 0 : target.eva;
    return (this.item().successRate * 0.01) + (hit - eva);
};
...
Game_Action.prototype.itemEva = function(target) {
    if(target.isActor()){
        ...
        var hit = this.subject().hit;
        var eva = (target.attackedFrom()=="back" || target.isStateAffected(8)) ? 0 : target.eva;
        //console.log( "eva:"+(1 - ((this.item().successRate * 0.01) + (hit - eva))) );
        return 1.0 - ((this.item().successRate * 0.01) + (hit - eva));
    }
    return 0;
};
/** Critical Hit - is based on user.hp - target.eva
Attacks from behind ignores target's eva
------------------------------------------------------------------------------------------*/
Game_Action.prototype.itemCri = function(target) {
    if (this.item().damage.critical) {
        var hit = this.subject().hit;
        var eva = (target.eva > 0 && (target.attackedFrom()=="back" || target.isStateAffected(8))) ? 0 : target.eva;
        return hit - eva;
    } else {
        return 0;
    }
...
Game_Action.prototype.applyCritical = function(damage) {
    return damage * (1.5 + this.subject().cev);
};
...
Game_Battler.prototype.enityTargeted = function(){
    return this._enityTargeted;
}

Game_Battler.prototype.setEnityTargeted = function(targets){
    this._enityTargeted = targets;
}
...
BattleManagerTBS.prepareDirectionalDamageBonus = function (user, targets, item) {
    ...
    user.battler().setEnityTargeted(targets.length);
};
...
/**** Counter rebuilt to add in attack range
Not finished - turns out it's more complex than I thought 
hint: how 2 make werid scopes? Also infinite loops.
**************************************************************************************/
Game_Battler.prototype.counterSkillId = function() {
    if(this.isStateAffected(73)) return 101;
    if(this.isStateAffected(74)) return 103;
    if(this.isStateAffected(75)) return 105;
    return 1;
};
BattleManagerTBS.counterRange = function(battler) {
    //if(battler.isStateAffected(74)) return 2;
    return 1;
};
BattleManagerTBS.forceAction = function (obj, entity) {
    var battler = entity.battler();
    ...
    this._activeAction = action; // this makes the action the main action
    this.updateTargetStatus();
    entity.lookAt(this._activeCell);
    entity.appendSequence(id, action);
    entity.checkDeath();
...
BattleManagerTBS.onActionEnd = function (entity) {
 ...
 // After force action a new action is created now
    this.newAction(this.activeBattler(), true);
    //this.activeAction()._item._dataClass = '';
    this.updateTargetStatus();
};
BattleManagerTBS.processCounterAttack = function (targets, subject, action) {
    if (!action) return;
    this.setCursorCell(subject.getCell()); 
    targets.forEach(function (entity) { // Can't counter allies or yourself; Can't counter on your turn
        if (entity && this.activeEntity() !== entity && !LeUtilities.isAlly(subject.battler(), entity.battler())) {
            var dist = LeUtilities.distanceBetweenCells(subject.getCell(), entity.getCell());
            var reqDist = this.counterRange(entity.battler());
            if (dist === reqDist && Math.random() < action.itemCnt(entity.battler())) {
...
var AdvDontPaySkillCostIfNotYourTurn = Game_BattlerBase.prototype.paySkillCost;
Game_BattlerBase.prototype.paySkillCost = function(skill) {
    if(BattleManagerTBS.activeEntity() !== this.getEntity()) return;
    AdvDontPaySkillCostIfNotYourTurn.call(this, skill);
};
BattleManagerTBS.forceActionInstant = function (obj, entity, cell) {
    var battler = entity.battler();
    var aoeData = obj.id === battler.attackSkillId() ? entity.getAttackAoEData() : entity.getObjectAoEData(obj);
    var param = this.makeObjAoEParam(obj, entity, cell);
    var aoe = this.getScopeFromData(aoeData, cell, param);
    
    aoe = aoe.filter(function (cell) {return cell.isThereEntity() && cell.getEntity()!=null;});
    
    var targets = aoe.map(function(e){return e.getEntity()});
    entity.lookAt(cell);
    this.setCursorCell(cell); 
    for(var i = 0; i < targets.length; i++){
        targets[i].newAnimation(obj.animationId, false, 10);
        BattleManagerTBS.applyFloatingDamage(1, targets[i]);
    }
    entity.checkDeath();
};
// Window attachment
...
/*
TBSEntity.prototype.attachWindow = function (win) {
    var x = this._posX - win.windowWidth()/2;// - win.windowWidth() / 2;
    var y = this._posY + win.windowHeight()/2;//- win.windowHeight();
    win.x = x + this.width() / 2;
    win.y = y;
    var maxW = $gameMap.width() * $gameMap.tileWidth();
    if (win.y < 0)
...
TBSEntity.prototype.onDeath = function () {
    ...
    // Although the turn order icon stays on death it will disappear on turn refresh
    this.hideMiniHud();
    BattleManagerTBS.wait(60);
    BattleManagerTBS.destroyEntity(this, false);
};
// this solves double popup on non-seq skills ... somehow
BattleManagerTBS.applyObjEffects = function (user, targets, hitAnim, animDelay) {
    targets.forEach(function (target) {
        target.prepareExtraPopups();
        if (Math.random() < this.activeAction().itemMrf(target.battler()))
...
TBSSequenceManager.prototype.commandPush = function (param) {
    ...
    var distance = Number(param[2]);
    var damage = param[3] !== "false";
    var includeSourceCell = param[4] !== "false";

    for (var i = 0; i < targets.length; i++) {
        var entity = targets[i];
        if(!includeSourceCell && sourceCell.getEntity()===entity) continue; 
        entity.forcePush(this.getUser(), sourceCell, distance, this.getObj(), damage);
    }
...
TBSEntity.prototype.forcePush = function (user, sourceCell, distance, obj, damage) {
    if (this.isImmuneToKnockback() && obj !== 'ignoreImmunity'){
        return;
    }
    if (!obj || obj === 'ignoreImmunity') { obj = $dataSkills[1]; }
    distance += user.getKnockbackBonus(obj) - this.getKnockbackReduction(obj);
    var old = this.getDir();
    this.lookAway(sourceCell);
    this.forceMoveStraight(distance, damage);
...
//----------------------------------------------------------------------------
// Bandage fixes that will most likely need to be steamlined
// This is what will probably makes this script not future proof
//----------------------------------------------------------------------------
Game_ActionResult.prototype.pushRemovedState = function(stateId) { // state can add and remove in the same turn
    //if (!this.isStateRemoved(stateId)) {
    //    this.removedStates.push(stateId);
    //}
};

/** Escape is 100% because escape is werid
---------------------------------------------------------------------*/
BattleManagerTBS.processEscape = function () {
    $gameParty.performEscape();
    var success = BattleManager._preemptive ? true : (Math.random() < BattleManager._escapeRatio);
    ...
    BattleManager._escaped = true; // Escape always succeeds
    SoundManager.playEscape();
    this.prepareAbort();
};

/** projectiles can be an from the icon sheet
---------------------------------------------------------------------*/
TBSProjectileSprite.prototype.readData = function (id) {
    var data = Lecode.S_TBS.Config.Projectiles[id];
    if (data.filename) {
        ...
        this.bitmap = new Bitmap(data.anim[1], data.anim[2]);
        this.startAnimation(data.anim[0]);
    } else if (data.weapicon) {
        var iconIndex = data.weapicon[0];//TBSSequenceManager.getUser().battler().weapons[0].iconIndex;
        ...
        this.bitmap = new Bitmap(data.weapicon[1], data.weapicon[2]);
...
//-----------------------------------------------------------------------------
// This preplace members index instead of actor id
//-----------------------------------------------------------------------------
BattleManagerTBS.processPositioningPhase = function () {
    this._phase = "positioning";
    this._subPhase = "";
    ...
    this.allyStartCells().forEach(function (cell) {
        if (cell._positioningData.type === "actor") {
            ...
            //}
            if (id && $gameParty.battleMembers()[id-1]){
                this._actorsToPositionate.push($gameParty.battleMembers()[id-1]); 
            }
        }
    }.bind(this));
    this.processActorsPrePositioning();
    ...
    InputHandlerTBS.setOnTouchCallback(this.positioningPhaseOnTouchInput.bind(this))
        ...
        .setOnUpCallback(this.positioningPhaseOnInputUp.bind(this))
        .setOnMoveCallback(this.positioningPhaseOnMouseMove.bind(this));
    if ($gameSwitches.value(5)) this.battleBeginning(); // MIB
};
...
BattleManagerTBS.processActorsPrePositioning = function () {
    ...
    do {
        ...
        var cell = this.allyStartCells().filter(function (cell) {
            //return cell._positioningData.fixed === actor.actorId();
            return cell._positioningData.fixed-1 === $gameParty.battleMembers().indexOf(actor); // MAB
        })[0];
        var entity = new TBSEntity(actor, this.getLayer("battlers"));
        entity.setCell(cell);
        this._battlerEntities.push(entity);
...
TBSEntity.prototype.filenameID = function () {
    var battler = this.battler();
    var def = battler.isActor() ? battler.name() : battler.originalName();
    if (battler.isStateAffected(23)){
        def += "_beast";
    }
    return battler.getLeTBSTagStringValue("spriteName", def);
};*/
//-----------------------------------------------------------------------------
// This creates "Pause" buff in the game
//-----------------------------------------------------------------------------
var GBB_updateStateTurns = Game_BattlerBase.prototype.updateStateTurns;
Game_BattlerBase.prototype.updateStateTurns = function() {
    if (this.isStateAffected(13)){ // Pause
        this._states.forEach(function(stateId) {
            if ( this._stateTurns[stateId] > 0 && 
              !($dataStates[stateId].category.contains('BUFF') || $dataStates[stateId].category.contains('DEBUFF'))
               ) {
        ...
        GBB_updateStateTurns.call(this);
    }
};
//-----------------------------------------------------------------------------
// This clears all state counters and recover all status
//-----------------------------------------------------------------------------
var MB_BMTBS_stopBattle = BattleManagerTBS.stopBattle;
BattleManagerTBS.stopBattle = function () {
    $gameParty.members().map(function(a){
        a.clearStateCounters();
        a.setTp(0);
        //a.recoverAll();
        a.setupTempStat();
    });
    MB_BMTBS_stopBattle.call(this);
};

//-- Aura changed. This allows auras from different states to stack together
//-------------------------------------------------------------------------------
Game_Battler.prototype.getLeTBSTag = function (str) {
    var i;
    //-- Get the tag from the battlers tag effects (first priority)
...
/** This centers the battle text at the beginning of battle
Bandage patch
---------------------------------------------------------------------*/
BattleManagerTBS.battleBeginning = function () {
    this._phase = "battle_beginning";
    ...
    //this._startSprite.x = Graphics.width / 2 - this._startSprite.width / 2;
    //this._startSprite.y = Graphics.height / 2 - this._startSprite.height / 2;
    this._startSprite.x = Graphics.width / 2 - 672 / 2;
    this._startSprite.y = Graphics.height / 2 - this._startSprite.height / 2;
    this._startSprite.opacity = 0;
    this._subPhase = "in";
};
/** This attaches the battle status window to a switch
Allows cleaner "cutscene" in battles.
---------------------------------------------------------------------*/
Scene_Battle.prototype.showStatusWindow = function (entity, slide) {
    return;
...
/* This attaches the battle status window to a switch This also adds items to enemies and add common event to battles
---------------------------------------------------------------------*/
/*
var ADV_TBSEntity_prototype_onBattleStart = TBSEntity.prototype.onBattleStart;
TBSEntity.prototype.onBattleStart = function () {
    ADV_TBSEntity_prototype_onBattleStart.call(this);
    if (this.battler().hasLeTBSTag("medicine")) this.battler().addState(36);
    if (this.battler().hasLeTBSTag("spiritbottle")) this.battler().addState(37);
    if (this.battler().hasLeTBSTag("cureall")) this.battler().addState(38);
};*/

/* This updates battle end
---------------------------------------------------------------------*/
BattleManagerTBS.updateBattleEnd = function () {
    BattleManager.updateBattleEnd();/* 
    if (!BattleManager._escaped && !BattleManagerTBS.canPrepareVictory()) {
        if (BattleManager._canLose) {
            //$gameSystem.onBattleEscape();
            //$gameParty.reviveBattleMembers();
            //SceneManager.pop();
        } else {
            // If not escaped game over.
            SceneManager.goto(Scene_Gameover);
        }
    }*/
    // MAB I'd hate to do this but force run common event 5
    $gameTemp.reserveCommonEvent(5); 
    this._phase = null;
    this.setStartBattleSwitch(false); // turn on battle start switch
};

/* Damage Preview window
Creates an unorthodox link between the editor, YEP, and LeTBS
- If you are planning to copy this, I recommend you carefully read each method
- I used "makeDamageValue()" to trigger all yanfly's scripts. Damage formulas 
must use "if(!BattleManagerTBS._damagePreview){}" to guard preview effect triggers
---------------------------------------------------------------------*/
var DamagePreview_BattleManagerTBS_initMembers = BattleManagerTBS.prototype.initMembers;
BattleManagerTBS.prototype.initMembers = function () {
    DamagePreview_BattleManagerTBS_initMembers.call(this);
    this._damagePreview = false;
};

/*
var DamagePreview_BattleManagerTBS_touchActionSelection = BattleManagerTBS.touchActionSelection;
BattleManagerTBS.touchActionSelection = function (selectedCell) {
    this.setCursorCell(selectedCell);
    if(selectedCell.getEntity()!=null){
        this._damagePreview = true;
        var value = this.activeAction().makeDamageValue(selectedCell.getEntity().battler(), false);
        //console.log(value);
        LeUtilities.getScene()._damageWindow.drawTextEx(value+"",0,0);
        this._damagePreview = false;
    }
    DamagePreview_BattleManagerTBS_touchActionSelection.call(this,selectedCell);
};*/

Window_TBSStatus.prototype.getWidth = function () {
    return this.width;
};

Window_TBSStatus.prototype.getHeight = function () {
    return this.height;
};

Window_TBSStatus.prototype.setWidth = function (value) {
    this.width = value;
};

Window_TBSStatus.prototype.setHeight = function (value) {
    this.height = value;
};

var DamagePreview_Scene_Battle_prototype_createStatusWindow = Scene_Battle.prototype.createStatusWindow;
Scene_Battle.prototype.createStatusWindow = function () {
    DamagePreview_Scene_Battle_prototype_createStatusWindow.call(this);
    if (Lecode.S_TBS.commandOn) {
...
Scene_Battle.prototype.createPreviewWindow = function () {
    //var help = this._helpWindow.height;
    this._previewWindow1 = new Window_TBSStatus();
    this._previewWindow1.x = 0;
    //this._previewWindow1.y = 0;
    this._previewWindow1.y = Graphics.height - this._previewWindow1.height;
    this._previewWindow1.hide();
    
    this._damageWindow = new Window_Base();
    //Window_Base.prototype.initialize.call(this._damageWindow, 0, this._previewWindow1.height, 320, 72);
    Window_Base.prototype.initialize.call(this._damageWindow, 0, 0, 320, 72);
    this._damageWindow.x = Graphics.width/2 - this._damageWindow.width/2;
    this._damageWindow.y = Graphics.height - this._damageWindow.height;
    this._damageWindow.hide();
    
    this._previewWindow2 = new Window_TBSStatus();
    //this._previewWindow2.x = this._previewWindow1.x;
    //this._previewWindow2.y = this._previewWindow1.y + this._previewWindow1.height + this._damageWindow.height;
    this._previewWindow2.x = Graphics.width - this._previewWindow2.width;
    this._previewWindow2.y = this._previewWindow1.y;
    this._previewWindow2.hide();
    this.addWindow(this._damageWindow);
    this.addWindow(this._previewWindow1);
    this.addWindow(this._previewWindow2);
};

Scene_Battle.prototype.showPreviewWindow = function (entity, target, slide) {
    if($gameSwitches.value(9)) return;
    //if (!entity || !target) return;
    if (entity.battler().rpgObject().TagsLetbs.hideStatusWindow) return;
    if (entity.battler().isDead() && entity.hasNoCorpse()) return;
    this._previewWindow1._entity = entity;
    this._previewWindow1.show();
    this._previewWindow1.open();
    this._previewWindow1.refresh();
    
    if (!target || target.battler().rpgObject().TagsLetbs.hideStatusWindow) return;
    if (target.battler().isDead() && target.hasNoCorpse()) return;
    if(target && BattleManagerTBS.activeAction()._item._dataClass == 'skill' && BattleManagerTBS.activeAction()._item._itemId){
        BattleManagerTBS._damagePreview = true;
        
        var value = BattleManagerTBS.activeAction().makeDamageValue(target.battler(), false);
        var hit = BattleManagerTBS.activeAction().itemHit(target.battler());
        hit = hit.clamp(0,200);
        BattleManagerTBS._damagePreview = false;
        
        var b = target.battler();
        var a = entity.battler();
        
        if(value > 0 && b.isStateAffected(10)){ // Rage
            value = Math.floor(value * 1.5);
        }
        if(a.isStateAffected(10)){ // Rage
            value = Math.floor(value * 1.5);
        }
        if(value > 0 && b.isStateAffected(14)){ // Defense
            var de = 1 - (b._stateTurns[14] * 0.10);
            value *= Math.min(de,value);
            value = Math.floor(value);
        }
        if(a.isStateAffected(17)){ // Strength
            var at = 1 + (a._stateTurns[17] * 0.15);
            value *= at;
            value = Math.floor(value);
        }
        
        this._damageWindow.drawTextEx("\\is["+BattleManagerTBS.activeAction()._item._itemId+"]",0,0);
        this._damageWindow.drawTextEx("\\i[104]"+value+"  "+(Math.round(hit*1e3)/1e1)+"%",this._damageWindow.width/2,0);
        this._damageWindow.update();
        this._damageWindow.show();
        this._damageWindow.open();
    }
    if(entity !== target || this._damageWindow.visible){
        this._previewWindow2._entity = target;
        this._previewWindow2.show();
        this._previewWindow2.open();
        this._previewWindow2.refresh();
        if (slide){
            this._previewWindow1.slide();
            this._previewWindow2.slideRight();
        }
    }
};
Scene_Battle.prototype.showHelpWindow = function () {
    ...
    help.x = 0;
    help.y = Graphics.height - help.height;
    this._damageWindow.y = Graphics.height - this._damageWindow.height - help.height;
    this._previewWindow1.y = Graphics.height - this._previewWindow1.height - help.height;
    this._previewWindow2.y = Graphics.height - this._previewWindow2.height - help.height;
    help.visible = true;
};

Scene_Battle.prototype.hideHelpWindow = function () {
    var help = this._helpWindow;
    help.y = Graphics.height;
    this._damageWindow.y = Graphics.height - this._damageWindow.height;
    this._previewWindow1.y = Graphics.height - this._previewWindow1.height;
    this._previewWindow2.y = Graphics.height - this._previewWindow2.height;
    help.visible = false;
};
var DamagePreview_BattleManagerTBS_updateTargetStatus = BattleManagerTBS.updateTargetStatus;
BattleManagerTBS.updateTargetStatus = function () {
    //DamagePreview_BattleManagerTBS_updateTargetStatus.call(this);
    var cell = this._activeCell;
    var entity = cell.getEntity();
    var scene = LeUtilities.getScene();
    scene._damageWindow.contents.clear();
    scene._damageWindow.update();
    scene._damageWindow.hide();
    scene._previewWindow1.hide();
    scene._previewWindow2.hide();
    scene.showPreviewWindow(this.activeEntity(),entity,true);
};

var DamagePreview_BattleManagerTBS_onActionCancel = BattleManagerTBS.onActionCancel;
BattleManagerTBS.onActionCancel = function () {
    BattleManagerTBS.activeAction()._item._dataClass = '';
    DamagePreview_BattleManagerTBS_onActionCancel.call(this);
};

BattleManagerTBS.hidePreviewWindow = function () {
    var scene = LeUtilities.getScene();
    scene._damageWindow.hide();
    scene._previewWindow1.hide();
    scene._previewWindow2.hide();
};
/* Scroll through status pages
---------------------------------------------------------------------*/
BattleManagerTBS.processBattle = function () {
    InputHandlerTBS.setActive(true);
    this._phase = "battle_processing";
    ...
    this.setEntitiesFlag();
    this.executeEventsWhen("battle_start");
    this.setStartBattleSwitch(true); // turn on battle start switch
    this.startTurn();
    ...
    InputHandlerTBS.setOnTouchCallback(this.battlePhaseOnTouchInput.bind(this))
        ...
        .setOnDownCallback(this.battlePhaseOnInputDown.bind(this))
        .setOnLeftCallback(this.battlePhaseOnInputLeft.bind(this))
        .setOnPageupCallback(this.battlePhaseOnInputPageup.bind(this))
        .setOnPagedownCallback(this.battlePhaseOnInputPagedown.bind(this));
};
InputHandlerTBS.setOnPageupCallback = function (callback) {
    this._onPageUpCallback = callback;
    return this;
};
InputHandlerTBS.setOnPagedownCallback = function (callback) {
    this._onPageDownCallback = callback;
    return this;
};
BattleManagerTBS.battlePhaseOnInputPageup = function () {
    this.Pageup();
};
BattleManagerTBS.battlePhaseOnInputPagedown = function () {
    this.Pagedown();
};
BattleManagerTBS.Pagedown = function() {
    SoundManager.playCursor();
    LeUtilities.getScene()._windowStatus.cursorPagedown();
    LeUtilities.getScene()._previewWindow1.cursorPagedown();
    LeUtilities.getScene()._previewWindow2.cursorPagedown();
    this.refreshBattlersStatus();
};
BattleManagerTBS.Pageup = function() {
    SoundManager.playCursor();
    LeUtilities.getScene()._windowStatus.cursorPageup();
    LeUtilities.getScene()._previewWindow1.cursorPageup();
    LeUtilities.getScene()._previewWindow2.cursorPageup();
    this.refreshBattlersStatus();
};

/* Adds extra functions to forceMove: getEntityByActorMember
---------------------------------------------------------------------*/
BattleManagerTBS.getEntityByActorMember = function (actorId) {
    var entities = this.allyEntities();
    actorId = $gameParty._battleMembers[actorId-1];
    for (var i = 0; i < entities.length; i++) {
        var entity = entities[i];
        if (entity.battler().actorId() === actorId)
            return entity;
...
/* Autosaving feature: When saving save file id into variable 12
This code literally does not belong here
---------------------------------------------------------------------*/
Scene_Save.prototype.onSavefileOk = function() {
    Scene_File.prototype.onSavefileOk.call(this);
    $gameSystem.onBeforeSave();
    $gameVariables.setValue(12,this.savefileId());
...
/* Autofixes running out of tp during movement 
- Moves to nearest empty pathable square
---------------------------------------------------------------------*/
var ifOnAnotherEntityPushAwayUpdate = BattleManagerTBS.onActiveEntityMoveEnd;
BattleManagerTBS.onActiveEntityMoveEnd = function () {
    ifOnAnotherEntityPushAwayUpdate.call(this);
    var entity = this.activeEntity();
    var allEntities = BattleManagerTBS.allEntities();
    for(var i = 0; i < allEntities.length; i++){
        if(entity === allEntities[i]) continue;
        if(entity._cellX === allEntities[i]._cellX && entity._cellY === allEntities[i]._cellY){
            var center = this.activeEntity().getCell();
            var maxRange = Infinity;
            var range = 1;
            var reachables = [];
            var foundCell = null;
            do {
                var scope = this.makeCircleScope(center, range, null, {});
                scope = LeUtilities.sortByClosestDistance(center, scope);
                for (var i = 0; i < scope.length; i++) {
                    var cell = scope[i];
                    if (BattleManagerTBS.isCellReachable(cell, maxRange, center, reachables)) {
            ...
            this._activeCell.getEntity().teleport(foundCell);
        }
    }
};