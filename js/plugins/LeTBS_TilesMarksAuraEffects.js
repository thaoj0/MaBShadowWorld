    ...
    for (var i = 0; i < marks.length; i++) {
        ...
        if (mark._user === entity) {
            ...
            for (var j = 0; j < entities.length; j++) {
                ...
                this.executeMarkEffects(mark, entity, "caster_turn" + occasion);
            }
...
TBSMarksManager.prototype.updateDuration = function (entity, occasion) {
    ...
    }// After marking marks, delete them
    marksToRemove.forEach(function (mark) {
        this.removeMark(mark._id, mark._user, mark._unique);
    }.bind(this));
};
...
TBSMarksManager.prototype.removeMarksOfEntity = function (entity) {
    ...
    marksToRemove.forEach(function (mark) {
        this.removeMark(mark._id, mark._user);
    }.bind(this));
};
...
TBSMarksManager.prototype.newMark = function (id, user, cell) {
    ...
    if (data) {
        if (!this.canAddMark(id, data)) {
            this.removeMark(id, user);
        }
        if (id.slice(-5) === "field") {
            this.fieldMarksCantStack(cell);
            // Enemy id == 11 is the neutral that holds all of the fields
            user = BattleManagerTBS.allEntities().filter(function(e){return e._battler._enemyId === 11}).pop();
        }
        var mark = new TBSMark(id, data, user, cell);
        mark._unique = ++this._markCounter;
...
TBSMarksManager.prototype.removeMark = function (id, entity, unique) {
    unique = unique||0;
    //console.log("removed "+unique);
    this._marks = this._marks.filter(function (mark) {
        if (mark._id === id && mark._user === entity && (unique === 0 || mark._unique === unique)) {
...
/** This rebuilds the caster turn to be more clear
---------------------------------------------------------------------------------*/
BattleManagerTBS.processMarkEffectsWhenCasterTurn = function (entity, occasion) {
    var marks = this._marksManager._marks;
    for (var i = 0; i < marks.length; i++) {
        var mark = marks[i];
        if (mark._user === entity) { // entity is the caster
            var aoe = mark.getSizeArea();
            var entities = this.getEntitiesInScope(aoe);
            for (var j = 0; j < entities.length; j++) {
                //var entity = entities[j];
                //this.executeMarkEffects(mark, entity, "caster_" + occasion);
                var target = entities[j]; // local recasting: fix = changed entity to target
                this.executeMarkEffects(mark, target, "caster_" + occasion); // turn_ will trigger caster_turn_start 
            }

        }
    }
};
/** processes auras at the beginning of battle
---------------------------------------------------------------------------------*/
var MB_TilesMarksAura_TonBattleStart = TBSEntity.prototype.onBattleStart;
TBSEntity.prototype.onBattleStart = function () {
    MB_TilesMarksAura_TonBattleStart.call(this);
    var auras = this.battler().collectLeTBSTags("auras");
    if (auras.length > 0) BattleManagerTBS.processAurasEffectsWhenApplied();
};


/** More functions added to control marks 
---------------------------------------------------------------------------------*/
BattleManagerTBS.getFieldInCell = function (cell) {
    switch (cell.getMark()._id){
        case "burn_field": return 1;
        case "soak_field": return 2;
        case "poison_field": return 3;
        case "confuse_field": return 4;
        case "daze_field": return 5;
        case "curse_field": return 6;
    }
    return 0;
};

BattleManagerTBS.getMarkInCell = function (cell) {
    return cell.getMark();
};

BattleManagerTBS.getMarksInScope = function (scope) {
    var entities = [];
    for (var i = 0; i < scope.length; i++) {
        var cell = scope[i];
        var entity = cell.getMark();
        if (entity)
...
TBSCell.prototype.getMark = function () {
    var entities = BattleManagerTBS._marksManager._marks;
    for (var i = 0; i < entities.length; i++) {
        var entity = entities[i];
...
TBSMark.prototype.getCell = function () {
    return this._cell;
};

TBSMarksManager.prototype.removeMarkAt = function (id, cell) {
    this._marks = this._marks.filter(function (mark) {
        if (mark._id === id && mark._cell === cell) {
            mark.stopAnimation(this._layer);
            var anim = mark._data.disappearing_anim;
            BattleManagerTBS.getLayer("animations").newAnimation(anim, false, 0, cell);
            return false;
...
/** Remove fields at cell location. Fields are mark._id ending with "field"
---------------------------------------------------------------------------------*/
TBSMarksManager.prototype.removeFieldAt = function (cell) {
    var markDeleted = null;
    this._marks = this._marks.filter(function (mark) {
        if (mark._id.slice(-5) === "field" && mark._cell === cell) {
            mark.stopAnimation(this._layer);
            var anim = mark._data.disappearing_anim;
            BattleManagerTBS.getLayer("animations").newAnimation(anim, false, 0, cell);
            markDeleted = mark;
...
/** Remove fields at saved cell location. Fields are mark._id ending with "field". Saved cells are on id "remove_field_saved"
---------------------------------------------------------------------------------*/
TBSMarksManager.prototype.removeFieldSaved = function () {
    var markDeleted = null;
    var cell = BattleManagerTBS.activeEntity()._sequenceManager._savedCells["remove_field_saved"]||[];
    for(var i = 0; i < cell.length; i++){
        this._marks = this._marks.filter(function (mark) {
            if (mark._id.slice(-5) === "field" && mark._cell === cell[i]) {
                mark.stopAnimation(this._layer);
                var anim = mark._data.disappearing_anim;
                BattleManagerTBS.getLayer("animations").newAnimation(anim, false, 0, cell[i]);
                markDeleted = mark;
                return false;
            }
            return true;
...
/** When adding a new field on top of a old one it replaces it
---------------------------------------------------------------------------------*/
TBSMarksManager.prototype.fieldMarksCantStack = function (cell) {
    var marks = this._marks;
    for(var i = 0; i < marks.length; i++){
        if (marks[i]._cell.x === cell.x && marks[i]._cell.y === cell.y) {
            this.removeFieldAt(marks[i]._cell);
            break;
        }
    }
};