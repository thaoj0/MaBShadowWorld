...
Window_ItemCategory.prototype.windowWidth = function() {
    // ----- MAB changed the size of the menu window -------------------------------
    if (SceneManager._scene instanceof Scene_Item) return 140; // return 240; 
    // -----------------------------------------------------------------------------
...
Window_ItemStatus.prototype.drawItemData = function(i, dx, dy, dw) {
    ...
    if (i === 0) {
      ...
      value = (effect) ? effect.value1 : emptyValue;
      if (value === 0) value = emptyValue;
      if (value !== emptyValue && value !== 0) value *= 100;
    }
    if (i === 1) {
      effect = this.getEffect(Game_Action.EFFECT_RECOVER_HP);
      value = (effect) ? effect.value2 : emptyValue;
      if (value === 0) value = emptyValue;
    }
    if (i === 2) {
...
/** Rebuilds the drawEquipInfo view
-------------------------------------------------------------------------------------*/
Window_ItemStatus.prototype.drawEquipInfo = function(item) {
    var rect = new Rectangle();
    if (eval(Yanfly.Param.ItemShowIcon)) {
    ...
    var array = [["\\i[1]MaxHP",item.params[0]],["\\i[39]Armor",item.params[1]],
                 ["\\i[2]MaxAP",item.params[6]],["\\i[9]Cost",item.params[7]]];
    for (var i = 0; i < array.length; ++i) {
      ...
      //this.drawText(TextManager.param(i), dx, rect.y, dw);
      this.drawTextEx(array[i][0], dx, rect.y);
      //this.changeTextColor(this.paramchangeTextColor(item.params[i]));
      var value = array[i][1];
      var cost = value;
      if (i===3) cost = -cost;
      this.changeTextColor(this.paramchangeTextColor(cost));
      //var text = Yanfly.Util.toGroup(item.params[i]);
...
/** Literally a crafting system
-------------------------------------------------------------------------------------*/
var ADV = ADV || {};
ADV.data = ADV.data || {};
ADV.getSum = function(total, num){return total+num;}; // Adds all number in array

ADV.writeToFile = function(text){
    var fs = require('fs');
    var stream = fs.createWriteStream("debug.txt");
...
ADV.readToFile = function(){
    var fs = require('fs');
    var stream = fs.createReadStream("debug.txt");
    stream.once('open', function(fd) {
        try{
            stream.read(0);
            stream.close();
...
Game_Party.prototype.literallyCrafting = function() {
    var ing = [$gameVariables.value(5),$gameVariables.value(6),$gameVariables.value(7),$gameVariables.value(8)];
    var empty = 4;
    var recipes = JsonEx.makeDeepCopy(ADV.data.Cooking_Recipes);
    var successRecipe = 0;
    var itemCounter = [];
    for(var i = 0; i < recipes.length; i++){
        if(!recipes[i]) continue;
        var recipe = recipes[i];
        // Push "empty" to max recipe slots
        while(recipe.length < 4) recipe.push(empty);
        itemCounter[ing[0]] = 0;
        itemCounter[ing[1]] = 0;
        itemCounter[ing[2]] = 0;
        itemCounter[ing[3]] = 0;
        // Recipe Match: itemCounter cause same item to stack
        ing.forEach(function(e){
            if(recipe.indexOf(e)>=0){
                recipe[recipe.indexOf(e)] = 0;
                itemCounter[e] += 1;
            }
        });
        
        if(recipe.reduce(ADV.getSum)!==0) continue;
        // Enough Ingredents by checking itemCounter. skip empty
        for(var j = 0; j < itemCounter.length; j++){
            if(!itemCounter[j]) continue;
            if(j===empty) continue;
            if(itemCounter[j] > (this.itemContainer($dataItems[j])[j]||0)){
                $gameMessage.addText("Not Enough Ingredients.\\autoevent[0]");
                return "notEnough"; 
            }
        }
        successRecipe = i;
    ...
    if(successRecipe>0) {
        for(var i = 0; i < ing.length; i++){
            this.loseItem($dataItems[ing[i]], 1);
        }
        this.gainItem($dataItems[successRecipe], 1);
        $gameMessage.addText("Crafted \\ii["+successRecipe+"]!\\autoevent[0]");
        return "success";
    }
    $gameMessage.addText("Wrong Ingredients.\\autoevent[0]");
    return "noMatch";
}

ADV.data.Cooking_Recipes = []; // 4 means empty
ADV.data.Cooking_Recipes[1] = [13,13,13];
ADV.data.Cooking_Recipes[2] = [14,14,13];
ADV.data.Cooking_Recipes[3] = [13,13];