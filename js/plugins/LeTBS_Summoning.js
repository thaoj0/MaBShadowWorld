...
BattleManagerTBS.newSummon = function (caster, summonId, cell) {
    ...
    if (summonData.turn_order !== "none") this.updateTurnOrderForSummon(caster, summon, summonData.turn_order);
    summon.onSummoned(caster, summonData.stats);
    
    this._summons.forEach(function (entity) {
        if (summon._summonData === entity._summonData){ 
            // _summonId is increased by the amount of its summon on the battlefield.
            summon._summonId += 1;
        }
    });
};
