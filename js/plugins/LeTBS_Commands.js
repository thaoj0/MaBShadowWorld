...
/** Battle Menu Command Menu
--------------------------------------------------------------------------*/
Window_TBSCommand.prototype.maxCols = function () {
    return 5;
};
...
Window_TBSCommand.prototype.makeCommandList = function () {
    if (this._battler) {
        this.makeIcons(["skill 34","skill 7","move 66","skill 8","pass 38"]);
        this.addCommand("", 'skill', this._entity.canSkillCommand(), 3);
        this.addCommand("", 'skill', this._entity.canSkillCommand(), 1);
        this.addCommand("", "move", this._entity.canMoveCommand());
        this.addCommand("", 'skill', this._entity.canSkillCommand(), 2);
...
Window_TBSCommand.prototype.cursorDown = function() {
    BattleManagerTBS.Pagedown();
};

Window_TBSCommand.prototype.cursorUp = function() {
    BattleManagerTBS.Pageup();
};*/

Window_TBSCommand.prototype.scrollDown = function() {
    BattleManagerTBS.Pagedown();
};

Window_TBSCommand.prototype.scrollUp = function() {
    BattleManagerTBS.Pageup();
};
/** Below rebuilds the command window to be 3x3
--------------------------------------------------------------------------*/
/*
Window_TBSCommand.prototype.maxCols = function () {
    return 3;
...
//Window_TBSCommand.prototype.standardBackOpacity = function() {
//    return 0;
//};
// New method create empty commands
Window_TBSCommand.prototype.addCommand_blank = function () {
    this.addCommand("", "nothing", false);
};
Window_TBSCommand.prototype.makeCommandList = function () {
    if (this._battler) {
        this.makeIcons(["","skill 113","",
                        "skill 116","move 172","skill 117",
                        "","pass 237",""]);
        
        this.addCommand_blank();
        this.addCommand("", 'skill', this._entity.canSkillCommand(), 3);
        this.addCommand_blank();
        this.addCommand("", 'skill', this._entity.canSkillCommand(), 1);
        this.addCommand("", "move", this._entity.canMoveCommand());
...
// command last is always on move index 4
Window_TBSCommand.prototype.selectLast = function () {
    this.select(4);
    //if (this._battler) {
    //    var last = this._battler.lastCommandSymbol();
...
Window_TBSCommand.prototype.cursorDown = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (index < maxItems - maxCols || (wrap && maxCols === 1)) {
        var post = (index + maxCols) % maxItems;
        if (post == 0||post == 2||post == 6||post == 8) return;
        this.select(post);
    }
};

Window_TBSCommand.prototype.cursorUp = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (index >= maxCols || (wrap && maxCols === 1)) {
        var post = (index - maxCols + maxItems) % maxItems;
        if (post == 0||post == 2||post == 6||post == 8) return;
        this.select(post);
    }
};

Window_TBSCommand.prototype.cursorRight = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (maxCols >= 2 && (index < maxItems - 1 || (wrap && this.isHorizontal()))) {
        var post = (index + 1) % maxItems;
        if (post == 0||post == 2||post == 6||post == 8) return;
        this.select(post);
    }
};

Window_TBSCommand.prototype.cursorLeft = function(wrap) {
    var index = this.index();
    var maxItems = this.maxItems();
    var maxCols = this.maxCols();
    if (maxCols >= 2 && (index > 0 || (wrap && this.isHorizontal()))) {
        var post = (index - 1 + maxItems) % maxItems;
        if (post == 0||post == 2||post == 6||post == 8) return;
        this.select(post);
    }
};
*/
/** _page are added to windowStatus and controlled by command
------------------------------------------------------------------------*/
Window_TBSCommand.prototype.cursorPagedown = function() {
    BattleManagerTBS.Pagedown();
};

Window_TBSCommand.prototype.cursorPageup = function() {
    BattleManagerTBS.Pageup();
};