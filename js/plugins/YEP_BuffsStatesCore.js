...
var ADV = ADV || {};
ADV.data = ADV.data || {};
ADV.data.linkCounterState = [
        [60,'eva',0.1], // Trickster
        [69,'eva',-0.1], // Bloodstain
        [96,'maxmp',5], // Justice
        [102,'hit',0.10], // Magic Magizine
        [73,'atk',1], // Sword
        [74,'atk',1], // Spear
        [75,'atk',1], // Mace
        [28,'atk',1], // Weapon
        [28,'def',1], // Weapon
        [71,'hit',0.05], // ActiveHit
        [110,'luk',1], // Crystal
        [110,'agi',1], // Crystal
        [72,'eva',0.05] // MobileEvasion
];
ADV.data.linkItemState = [
    [1,36],[2,37],[3,38],[5,47],[17,112],[18,113],[19,114],[20,115],[21,116],[22,117]
];

/**  Controls default turn count
----------------------------------------------------------------------------*/
var MB_GBB_resetStateCounts = Game_BattlerBase.prototype.resetStateCounts;
Game_BattlerBase.prototype.resetStateCounts = function(stateId) {
    MB_GBB_resetStateCounts.call(this, stateId);
    var buff = $dataStates[stateId].category.contains('BUFF');
    var debuff = $dataStates[stateId].category.contains('DEBUFF');
    if (this.isStateAffected(57) && (buff||debuff)) this._stateTurns[stateId] += 1; // Virus
    if (this.isStateAffected(7) && (debuff)) this._stateTurns[stateId] += 1; // Curse
    if (this.isStateAffected(6) && (buff)) this._stateTurns[stateId] -= 1; // Daze
    if (this._stateTurns[stateId] == 0) this.removeState(stateId);
};
// This attaches state counters to certain stat using tempStat
Game_BattlerBase.prototype.setStateCounter = function(stateId, value) {
    if (this._stateCounter === undefined) this.initStateCounter();
    if (value>0 && !this.isStateAffected(stateId)) this.addState(stateId); // adding counter will add the state
    this._stateCounter[stateId] = value;
    
    var link = ADV.data.linkCounterState;
    // Efficient Note: Have to check those states even if it's 0
    for(var i = 0; i < link.length; i++){
        var e = link[i];
        var stat = e[1];
        var key = e[1]+e[0];
        var value = e[2]*(this.getStateCounter(e[0])||0);
        this.setTempStat( stat, [key, value] );
        //console.log(e[2]+"*"+this.getStateCounter(e[0])+"="+value);
        //console.log(stat+", ["+key+", "+value+"]");
    }
    //console.log(this.getTempStat('eva'));
    
    this.refresh();
};

/** This creates a new methed called battler().oldStates()
Notes: This saves both attacker's and defender's "old states"
"new states" are current states before any "action".

This also creates a new method called battler().justAddedStates()
-------------------------------------------------------------------------*/
Game_BattlerBase.prototype.oldStates = function() {
    return this._preStates;
};
Game_BattlerBase.prototype.justAddedStates = function() {
    return this._justAdded;
};
Game_BattlerBase.prototype.justAddedBD = function(category) {
    var buffs = [];
    category = category.toUpperCase().trim();
    var states = this.justAddedStates();
    for(var i = 0; i < states.length; i++){
        var state = $dataStates[states[i]];
        if (!state) continue;
        if (state.category.contains(category.toUpperCase())) buffs.push(state); 
    }
    return buffs;
};

Game_BattlerBase.prototype.setPreActionStates = function() {
    this._preStates = this.states();
    this._justAdded = [];
};
Game_Action.prototype.initiateStateEffects = function(target, stateId) {
    target.setPreActionStates();
    this.subject().setPreActionStates();
    this.customEffectEval(target, stateId, 'initiateState', this.subject());
};
Game_Battler.prototype.addStateEffects = function(stateId) {
    this._justAdded.push(stateId);
    this.customEffectEval(stateId, 'addState');
};
/** This creates a new methed called arrayStateCategory(category)
Notes: This returns the array of the particular category.
-------------------------------------------------------------------------*/
Game_Battler.prototype.arrayStateCategory = function(category, time) {
    var array = [];
    category = category.toUpperCase().trim();
    var currentStates = time == 1 ? this.states() : this.oldStates();
    var length = currentStates.length;
    for (var i = 0; i < length; ++i) {
      var state = currentStates[i];
      if (!state) continue;
      if (state.category.contains(category.toUpperCase())) array.push(state);
    }
    console.log(category.toUpperCase());
    return array;
};
/** This creates a new methed called stateDefaultTurn(stateId)
Notes: This returns the default turn of a state
-------------------------------------------------------------------------*/
Game_BattlerBase.prototype.stateDefaultTurn = function(stateId) {
    var state = $dataStates[stateId];
    var variance = 1 + Math.max(state.maxTurns - state.minTurns, 0);
    return state.minTurns + Math.randomInt(variance);
};

/* Refreshes game battler
- Remove state if stateCounter is 0
---------------------------------------------------------------------*/
var ADV_Game_Battler_prototype_refresh = Game_Battler.prototype.refresh;
Game_Battler.prototype.refresh = function() {
    ADV_Game_Battler_prototype_refresh.call(this);
    var stateArray = [28,29,71,72,36,37];
    for(var i = 0; i < stateArray.length; i++){
        if(this.isStateAffected(stateArray[i]) && this.getStateCounter(stateArray[i]) === 0){ 
            this.removeState(stateArray[i]);
        }
    }
};
...
Game_BattlerBase.prototype.getStateCounter = function(stateId) {
    ...
    if(this.isActor()) {
        for(var i = 0; i < itemToState.length; i++){
            if(itemToState[i][1]==stateId){
                return this.actorItem('item '+itemToState[i][0]);
            }
        }
    }
    return this._stateCounter[stateId];
