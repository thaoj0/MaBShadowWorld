...
// End of File
//=============================================================================
// This pre-equip unattachable skills to main weapons
Game_Party.prototype.preEquipNewItem = function(item) {
    if (!DataManager.isWeapon(item)) return;
    var preEquipNewItem_control = [[],
        [101], // Sword
        [103], // Spear
        [105], // Mace
        [112,113], // Bow
    ...
    var control = preEquipNewItem_control[item.baseItemId];
    for (var i = 0; i < control.length; i++){
        ItemManager.applyAugmentEffects(item, $dataItems[control[i]], i);
    }
};
// This attaches items to any equipment on empty slot. Returns true if attach successful
Game_Party.prototype.itemAttachAugument = function(item, attach) {
    ItemManager.checkAugmentSlots(item);
    for (var i = 0; i < item.augmentSlotItems.length; i++){
        if (ItemManager.augmentInSlot(item, i) === null){
            ItemManager.applyAugmentEffects(item, attach, i);
            return true;
        }
    }
    return false;
};
/** Calling method - grab actor's equipped item bags - "item bags" are any item with Item slots
    Returns an array of all actor's equipped "item bags".
-------------------------------------------------------------------------------------*/
Game_Actor.prototype.actorBag = function() {
    var bags = [];
    for (var i = 0; i < this.equips().length; i++) {
        var equip = this.equips()[i];
        if (!equip) continue;
        if (!equip.baseItemId) continue;
        ItemManager.checkAugmentSlots(equip);
        for(var j = 0; j < equip.augmentSlots.length; j++){
            if(equip.augmentSlots[j].toUpperCase() === "ITEM"){
                bags.push(equip);
                break;
            }
        }
...
/** Calling method - grab actor's equipped item bags - "item bags" are any item with Item slots
    Returns a count of all actor's equipped "item".
-------------------------------------------------------------------------------------*/
Game_Actor.prototype.actorItem = function(item) {
    var count = 0;
    var bags = this.actorBag();
    for (var i = 0; i < bags.length; i++) {
        for (var j = 0; j < bags[i].augmentSlotItems.length; j++){
            if(bags[i].augmentSlotItems[j]==item) count++;
        }
    }
    return count;
};

/** Calling method - This auto pays ammo cost inside bags
-------------------------------------------------------------------------------------*/
Game_Battler.prototype.payAmmoCost = function(itemCode, stateId) {
    if(this.isActor()){
        var bags = this.actorBag();
        for(var i = 0; i < bags.length; i++){
            ItemManager.checkAugmentSlots(bags[i]);
            if(bags[i].augmentSlotItems.indexOf(itemCode) === -1) continue;
            var slotId = bags[i].augmentSlotItems.indexOf(itemCode);
            ItemManager.removeAugmentFromSlot(bags[i], slotId);
            ItemManager.installAugmentToSlot(bags[i], null, slotId);
            ItemManager.augmentRefreshParty(bags[i]);
            return;
        }
    }else{
        this.setStateCounter(stateId,this.getStateCounter(stateId)-1);
    }
}